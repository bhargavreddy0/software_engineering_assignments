...
app = FastAPI(title="Sandwich Maker API")
...
def read_one_order(order_id: int, db: Session = Depends(get_db)):
    obj = orders.read_one(db, order_id=order_id)
    if obj is None:
...
def update_one_order(order_id: int, order: schemas.OrderUpdate, db: Session = Depends(get_db)):
    if orders.read_one(db, order_id=order_id) is None:
        raise HTTPException(status_code=404, detail="Order not found")
...
def read_one_resource(resource_id: int, db: Session = Depends(get_db)):
    obj = resources.read_one(db, resource_id=resource_id)
    if obj is None:
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    obj = recipes.read_one(db, recipe_id=recipe_id)
    if obj is None:
...
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    obj = order_details.read_one(db, order_detail_id=order_detail_id)
    if obj is None:
